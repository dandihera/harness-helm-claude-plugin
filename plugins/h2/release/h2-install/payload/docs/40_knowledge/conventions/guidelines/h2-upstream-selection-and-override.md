...
source_references:
  - cookbooks/0600-upstream-integration/0605-upstream-selection-and-override.md
module:
  - docs
  - workflow
tags:
...
# Convention: h2 upstream selection and override

## Rule

기본 upstream provider/surface 선택의 기준은 `.harness-helm/h2-cartridge.yml`이다.
...
5. command mapping reference를 갱신한다 — runtime reference는 `references/upstream-tool-invocation.md`, source repository는 `0601 Upstream Tool Invocation` cookbook이 기준.
6. upstream surface recommendation reference를 갱신한다 — runtime reference는 `references/upstream-surface-map.md`, source repository는 `0603 Upstream Surface Map` cookbook이 기준.
...
promotion candidate가 생기면 바로 canonical 문서로 만들지 않고 `docs/40_knowledge/conventions/guidelines/h2-canonical-promotion-flow.md`의 승인 절차를 따른다.

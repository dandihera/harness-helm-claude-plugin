...
This reference is a runtime snapshot of upstream surface recommendations. It does not replace `.harness-helm/h2-cartridge.yml` or `references/cartridge-command-mapping.md` as the source of truth for provider, fallback label, or routing.
...
- Fallback labels and routing targets belong to `.harness-helm/h2-cartridge.yml` and `references/cartridge-command-mapping.md`.
- Upstream raw output must be normalized through `references/cartridge-output-normalization.md`.

...
이 reference는 upstream surface recommendation의 runtime snapshot입니다. Provider, fallback label, routing의 기준인 `.harness-helm/h2-cartridge.yml` 또는 `references/upstream-tool-invocation.md`를 대체하지 않습니다.
...
- Fallback label과 routing target은 `.harness-helm/h2-cartridge.yml`과 `references/upstream-tool-invocation.md`에 속합니다.
- Upstream raw output은 `references/upstream-output-normalization.md`를 통해 정규화해야 합니다.
